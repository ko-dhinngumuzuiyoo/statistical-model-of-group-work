...
# 5. 交互作用項の作成 (追加)
# ====================================================================
df["elderly_x_car"] = df["elderly_rate"] * df["car_per_1000"]
...
# 2. VIF (分散拡大係数) の計算 (交互作用項を含む)
X = df[["elderly_rate", "car_per_1000", "elderly_x_car"]] # 交互作用項を追加
...
print("\n### VIF (Variance Inflation Factor) 結果 (交互作用項を含む) ###")
print(vif_data)

# VIF > 10 のチェックとコメント
# 交互作用項を含む場合、個別の項のVIFは高くなりがちですが、ここでは元の項目のみをチェック
max_vif_main = vif_data[vif_data['Variable'].isin(['elderly_rate', 'car_per_1000'])]['VIF'].max()

print("\n--------------------------------------------------------------------------------")
if max_vif_main > 10.0:
    print(f"⚠️ **VIFの最大値 ({max_vif_main:.2f}) が 10 を超えています。**\n   交互作用項を含めたことで多重共線性がさらに強くなっている可能性があります。")
    print("   （交互作用項そのもののVIFが高くなるのは一般的です）")
else:
    print(f"✅ **VIFの最大値 ({max_vif_main:.2f}) は 10 以下です。**\n   多重共線性の深刻な問題はないと考えられます。")
print("--------------------------------------------------------------------------------")

# =========================
# 7. GLM (Poisson Regression) の推定 (更新)
# =========================
df["log_pop"] = np.log(df["population"])

# 説明変数を更新：主効果と交互作用項
X = df[["elderly_rate", "car_per_1000", "elderly_x_car"]] 
X = sm.add_constant(X) 
...
print("\n### GLM (Poisson Regression) 結果 (交互作用項あり) ###")
